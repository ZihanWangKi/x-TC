...
def cross_entropy_list(sources, targets, model, cache=None, batch=False, calculate=True):
    ...
    if batch:
        ...
        for i in tqdm(list(range(n_batches))):
            ce_list += cross_entropy_list(sources[i * batch:(i + 1) * batch], targets[i * batch:(i + 1) * batch], model,
                                          batch=False)
    ...
    print(logits, logits.shape, ce_list, labels, labels[:, 1:].contiguous().view(-1))
