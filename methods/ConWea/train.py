...
def main(dataset_path, print_flag=True):
    ...
    ##
    def generate_test_pseudo_labels(df, train_df, labels, label_term_dict, tokenizer):
        def argmax_label(count_dict):
            maxi = 0
            max_label = None
            for l in count_dict:
        ...
        for index, row in df.iterrows():
            ...
            if flag:
                ...
                y.append(lbl)
                # X.append(line)
                X.append(train_df.sentence[index])
                y_true.append(label)
        return X, y, y_true

    def test(df, train_df, test_df, labels, label_term_dict, label_to_index, index_to_label, dataset_path):
        ...
        X, y, y_true = generate_test_pseudo_labels(df, train_df, labels, label_term_dict, tokenizer)
        ...
        X_all = prep_data(texts=test_df["sentence"], max_sentences=max_sentences, max_sentence_length=max_sentence_length,
                          tokenizer=tokenizer)
        y_true_all = test_df["label"]
    ...
    ## for x-TC
    train_df = pickle.load(open(pkl_dump_dir + "df.pkl", "rb"))
    test_df = pickle.load(open(pkl_dump_dir + "test_df.pkl", "rb"))
    ##
    ...
    test(df, train_df, test_df, labels, label_term_dict, label_to_index, index_to_label, dataset_path)
