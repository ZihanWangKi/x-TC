...
python document_class_alignment.py --dataset_name ${dataset} --random_state ${seed} --lm_type ${lm}-${layer} --pca ${pca} --document_repr_type ${weight_type}-100 --cluster_method ${method}
...
python evaluate.py --dataset ${dataset} --stage Rep --suffix ${lm}-${layer}-${weight_type}-100
python evaluate.py --dataset ${dataset} --stage Align --suffix pca${pca}.clusgmm.${lm}-${layer}.${weight_type}-100.${seed}
python evaluate.py --dataset ${dataset} --stage final --suffix pca${pca}.clusgmm.${lm}-${layer}.${weight_type}-100.${seed}.0.5 --lm ${final_lm}
