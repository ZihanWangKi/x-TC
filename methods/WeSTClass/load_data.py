...
def load_test_dataset(dataset_name, vob, model='cnn', with_evaluation=True, truncate_len=None):
    if model == 'cnn':
        return load_test_cnn(dataset_name, vob, with_evaluation=with_evaluation, truncate_len=truncate_len)
    elif model == 'rnn':
        ...


def load_test_cnn(dataset_name, vob, with_evaluation=True, truncate_len=None):
    data_path = './' + dataset_name
    data, y = read_file(data_path, with_evaluation)
    ...
    for i in range(len(data)):
        ...
        for j in range(len(data[i])):
            if data[i][j] in vob and len(x[i]) < truncate_len:
                x[i].append(vob[data[i][j]])
        for j in range(len(x[i]), truncate_len):
            x[i].append(vob["<PAD/>"])
