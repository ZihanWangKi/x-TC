...
def run_method(args, train_set, test_set):
    if args.method == "X-Class":
        ...
        if args.additional_method == "ProtoCal":
            ...
            os.system("mkdir -p ../methods/X-Class/data/datasets/{}_test".format(args.dataset))
            os.system("cp {} ../methods/X-Class/data/datasets/{}_test/classes.txt".format(args.class_names_file, args.dataset))
            with open("../methods/X-Class/data/datasets/{}_test/dataset.txt".format(args.dataset), "w") as f:
                for line in test_set["x-TC"]:
                    f.write(line)
                    f.write("\n")
            with open("../methods/X-Class/data/datasets/{}_test/labels.txt".format(args.dataset), "w") as f:
                for line in test_set[args.label_name]:
                    f.write(str(line))
                    f.write("\n")
            os.system("cp {} ../methods/X-Class/data/datasets/{}_test/prompt.txt".format(args.prompt_file, args.dataset))
            os.chdir("../methods/X-Class/scripts")
        ...
        os.system("CUDA_VISIBLE_DEVICES={} python src/train.py --dataset_dir datasets/{}/ --test_file test.txt --test_label_file test_labels.txt --gpus 1 --train_batch_size 32 --accum_steps 4 --random_state {} {}"
                  .format(args.gpu, args.dataset, args.random_state, args.suffix))
        ...
        os.system("python pipeline.py --gpu {} --dataset {} --random_state {} {}".format(args.gpu, args.dataset, args.random_state, args.suffix))
