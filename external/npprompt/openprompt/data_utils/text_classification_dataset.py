...
class GeneralProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split):
        examples = []
        path = data_dir + '/{}.txt'.format(split)
        with open(path, "r") as fp:
            texts = list(map(lambda x: x.strip(), fp.readlines()))
        path = data_dir + '/{}_labels.txt'.format(split)
        text_labels = None
        if os.path.exists(path):
            with open(path, "r") as fp:
                text_labels = list(map(lambda x: x.strip(), fp.readlines()))
        for i in range(len(texts)):
            example = InputExample(guid=str(i), text_a=texts[i], label=0 if text_labels is None else int(text_labels[i]))
            examples.append(example)
        return examples
