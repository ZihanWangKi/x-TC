...
class LOTClassTrainer(object):
    ...
    def __init__(self, args):
        ...
        assert args.lm_type.startswith("bert")
        self.lm_type = args.lm_type  # 'bert-base-uncased'
        self.tokenizer = BertTokenizer.from_pretrained(self.lm_type, do_lower_case=True)
        ...
        self.model = LOTClassModel.from_pretrained(self.lm_type,
    ...
    def label_name_in_doc(self, doc):
        ...
        for i, wordpc in enumerate(doc):
            ...
            if idx >= self.max_len - 1:  # last index will be [SEP] token
                break
            if i == len(doc) - 1 or (self.lm_type.startswith("bert") and not doc[i + 1].startswith("##")) or doc[
    ...
    def write_results(self, loader_name="final_model.pt", out_file="out.txt"):
        ...
        pred = pred_labels.cpu().numpy().tolist()
        inference_path = self.dataset_dir.replace("datasets", "inference")
