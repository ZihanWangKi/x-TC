...
def main(rank, exp_name, cfg_file, visdom_env_name, TOTAL_ITR, seed, lm, clustering, test_mode):
    set_seed_all(seed)
    set_multi_GPUs_envs(rank, world_size)
    cfg_file_path = os.path.join(exp_name, 'config', cfg_file)
    ...
    sentence_all = Sentence_ALL(cfg, exp_name)
    ...
    keywords = KeyWords(cfg = cfg, logger = logger, exp_name = exp_name)
...
if __name__ == '__main__':
    ...
    spawn(main, args = (args.exp_name, cfg_file, visdom_env_name, args.total_iter, args.random_state, args.lm, args.cluster, args.test_mode),
